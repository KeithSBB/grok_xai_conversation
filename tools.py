...
from homeassistant.helpers.typing import StateChangedEvent
...
def make_json_serializable(
    ...
    if isinstance(obj, dict):
        ...
        return typing.cast(
            Union[str, int, float, bool, None], obj
        )  # Narrow non-container types
...
class GrokHaTool:
    ...
    _instance: Optional["GrokHaTool"] | None = None  # Singleton pattern
    _exposed_entities: Optional[List[Dict[str, Any]]] | None = (
        None  # Manual cache for async method
    )
    _debounce_call: Optional[CALLBACK_TYPE] | None = None  # For canceling debounce
    _registry_remove: Optional[CALLBACK_TYPE] | None = (
        None  # For unsubscribing registry listener
    )
    _state_remove: Optional[CALLBACK_TYPE] | None = (
        None  # For unsubscribing state listener
    )

    def __new__(cls, hass: HomeAssistant) -> "GrokHaTool":
        if cls._instance is None:
            cls._instance = super(GrokHaTool, cls).__new__(cls)
            cls._instance.hass = hass
            cls._instance.entity_registry = er.async_get(hass)
            cls._registry_remove = hass.bus.async_listen(
                "entity_registry_updated", cls._async_invalidate_cache
            )
            cls._state_remove = hass.bus.async_listen(
                "state_changed", cls._async_debounced_invalidate_cache
            )
        return cls._instance

    @classmethod
    async def _async_invalidate_cache(
        cls: Type["GrokHaTool"], event: StateChangedEvent
    ) -> None:
    ...
    @classmethod
    async def _async_debounced_invalidate_cache(
        cls: Type["GrokHaTool"], event: StateChangedEvent
    ) -> None:
        ...
        try:
            ...
            if cls._instance is None:
                _LOGGER.warning("Singleton not initialized; skipping debounce")
                return
            ...
            async def _delayed_invalidate(_now: datetime) -> None:
                ...
                _LOGGER.log(
                    getattr(logging, DEFAULT_LOG_LEVEL),
                    (
                        f"Debounced cache invalidation on state change for"
                        f" {event.data.get('entity_id', 'unknown')}"
                    ),
                )

            cls._debounce_call = async_call_later(
                cls._instance.hass, 30, _delayed_invalidate
            )
        except HomeAssistantError as e:
            _LOGGER.error(f"Error in debounced invalidation: {e}")

    @classmethod
    def async_unload_listeners(cls: Type["GrokHaTool"]) -> None:
        """Unload event listeners and reset cache.
        Call from async_unload_entry."""
    ...
    @classmethod
    def generate_tool_instructions(cls: Type["GrokHaTool"]) -> List[str]:
        """Generate tool instructions (moved from const.py to
        ...
        try:
            ...
            return [
                "'call_ha_service' with parameters: domain," "service, target, data"
            ]

    @classmethod
    @lru_cache(maxsize=1)
    def get_tool_schemas(cls: Type["GrokHaTool"]) -> List[Dict[str, Any]]:
        ...
        for name, method in inspect.getmembers(cls, predicate=inspect.isfunction):
            ...
            properties: Dict[str, Dict[str, Union[str, bool, Dict[str, Any]]]] = {}
            required = []
            for param_name, param in sig.parameters.items():
                ...
                schema: Dict[str, Union[str, bool, Dict[str, Any]]] = typing.cast(
                    Dict[str, Union[str, bool, Dict[str, Any]]], {"type": "string"}
                )
                ...
                if ann != inspect.Parameter.empty:
                    ...
                    if origin is Optional:
                        ...
                        if args:
                            item_schema = cls._build_json_schema(
                                args[0]
    ...
    def _build_json_schema(
        cls: Type["GrokHaTool"], ann: typing.Any
    ...
    async def call_ha_service(
        ...
        :example target: JSON object, e.g., {'entity_id': 'light.bedroom'}
         or {'area_id': 'living_room'}.
        :example data: JSON object for params, e.g., {'brightness_pct': 50,
         'rgb_color': [0, 255, 0], 'effect': 'rainbow'}.
        """
        try:
    ...
    async def get_exposed_entities(self) -> List[Dict[str, Any]]:
        ...
        if self._exposed_entities is None:
            try:
                ...
                )  # Cast to str; matches yield type
                ent_reg = er.async_get(self.hass)
                dev_reg = dr.async_get(self.hass)
                device_id_to_name = {
                    entry.id: entry.name or "" for entry in dev_reg.devices.values()
                }  # Coalesce None to ""
                exposed = []
    ...
    async def find_entities_advanced(
        ...
        if simple_mode:
            ...
            for ent in exposed:
                if domain_filter and ent["domain"] != domain_filter:
                    continue
                # Note: attr_filters require fresh attributes;
                #       defer to post-fetch if needed
                try:
                    if entity_globs_list and not any(
                        glob_match(ent["entity_id"], g) for g in entity_globs_list
                    ):
                        continue
            ...
            for entity_id in sorted(list(matching_entity_ids)):
                ...
                if attr_filters and not all(
                    fresh_state.attributes.get(k) == v for k, v in attr_filters.items()
                ):
                    continue
                ent = next(
                    (e for e in exposed if e["entity_id"] == entity_id),
            ...
            return (
                ...
                if matching
                else [{"message": "No entities found"}]
            )

        entity_globs = ensure_list(entity_globs)
        domains = (
            ensure_list(domains) or [filters.get("domain")]  # type: ignore
        ...
        floor_id_to_name = {
            entry.floor_id: entry.name or "" for entry in floor_reg.async_list_floors()
        }
        area_id_to_name = {
            entry.id: entry.name or "" for entry in area_reg.async_list_areas()
        }
        label_id_to_name = {
            entry.label_id: entry.name or "" for entry in label_reg.async_list_labels()
        }
        device_id_to_name = {
            entry.id: entry.name or "" for entry in dev_reg.devices.values()
        }
        ...
        for entry in all_entity_entries:
            ...
            # Entity ID glob
            if entity_globs and not any(
                glob_match(entry.entity_id, g) for g in entity_globs
            ):
                continue
            ...
            if area_globs:
                area_id = entry.area_id or (
                    dev_reg.async_get(entry.device_id).area_id
                    if entry.device_id
                    else None
                )
                area_name = area_id_to_name.get(area_id, "") if area_id else ""
                match_area = bool(area_name) and any(
                    glob_match(area_name, g) for g in area_globs
                )
            ...
            if floor_globs:
                ...
                if area_id:
                    ...
                    if area_entry and area_entry.floor_id:
                        ...
                        match_floor = bool(floor_name) and any(
                            glob_match(floor_name, g) for g in floor_globs
                        )
